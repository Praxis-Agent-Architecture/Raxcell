# Praxis TypeScript Middleware Integration For Raxcell 0.1.2
...
`0.1.2` contains the TypeScript client, protocol types, and a `raxcell` Node CLI with a Linux bubblewrap runner. Praxis can set `RAXCELL_BIN` to the installed `raxcell` bin or to a development build artifact such as `raxcell/sdk/dist/cli.js`.
...
For `0.1.2`, Praxis should resolve the CLI path from runtime config.
...
1. Add `@praxis-ai/raxcell@0.1.2` to Praxis.
