...
const backend = "macos-seatbelt";
const results = [];

function main() {
  const workspace = mkdtempSync(join(tmpdir(), "raxcell-macos-smoke-workspace-"));
  const externalRoot = mkdtempSync(join(tmpdir(), "raxcell-macos-smoke-external-"));
  ...
  try {
    ...
    const workspaceRequest = runRequest({
      workspace,
      argv: ["/bin/sh", "-lc", "mkdir -p raxcell_live_probe && printf 'raxcell-ok' > raxcell_live_probe/hello.txt && cat raxcell_live_probe/hello.txt && rm raxcell_live_probe/hello.txt"],
      policyGrants: [],
    });
    ...
    const externalWriteRequest = runRequest({
      workspace,
      argv: ["/bin/sh", "-lc", `printf raxcell-smoke > ${shellQuote(externalFile)}`],
      policyGrants: [],
    });
    ...
    const dynamicRequest = runRequest({
      workspace,
      argv: ["/bin/sh", "-lc", "cat $HOME/raxcell-smoke.txt"],
      policyGrants: [],
      env: { HOME: dirname(externalFile) },
    });
    ...
    if (ready) {
      const writeGrantRunRequest = runRequest({
        workspace,
        argv: ["/bin/sh", "-lc", `printf raxcell-smoke > ${shellQuote(externalFile)} && cat ${shellQuote(externalFile)}`],
...
function summarizePrepare(response) {
  return {
    ...
    rootRules: response?.backendArtifacts?.[0]?.data?.rootRules,
